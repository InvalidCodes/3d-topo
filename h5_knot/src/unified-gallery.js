 ...
 * 
 * 合并 knot-gallery.js 和 dataset-generator.js 的功能
 * 保持图一的优质渲染效果
 */
...
function buildChainGeometry({ rng, quality = 'mid', radius = 0.24 } = {}) {
  ...
  for (let i = 0; i < numLinks; i++) {
    ...
    const amp = rng ? 0.04 + rng() * 0.06 : 0.05;
    const waves = rng ? 2 + Math.floor(rng() * 4) : 3;
    const phase = rng ? rng() * Math.PI * 2 : 0;

    const curve = new PlanarWobbleCircleCurve({ radius: R, center, normal, waves, amp, phase });
...
function exportDataset() {
  ...
  for (let i = 0; i < count; i++) {
    dataset.samples.push({
      ...
      preset: presetId,
      difficulty: PRESETS[presetId]?.difficulty || 'unknown',
